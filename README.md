...
JIC does not bake data into the image. A curated, URL-verified manifest of 115 public-domain and freely-redistributable emergency documents lives in [sources.yaml](sources.yaml) — survival manuals, austere medicine, food preservation, water/power engineering, emergency comms, open textbooks, Kiwix ZIMs, and KA-Lite media packs.

Download content using tiered profiles:

```bash
# Default core emergency PDFs (~350 MB)
./helper-scripts/fetch-source-data.sh --profile core

# High-priority Kiwix emergency & medical ZIMs (~20 GB)
./helper-scripts/fetch-source-data.sh --profile emergency-zims

# Full encyclopedic ZIMs: Wikipedia Maxi, Gutenberg, WikiHow (~240 GB)
./helper-scripts/fetch-source-data.sh --profile full-zims

# Or inside Docker Compose
docker compose --profile fetch run --rm content-fetch
```

**Creating your own BitTorrent archives:**
To package large media sets or local documentation folders into `.torrent` files and magnet URIs:
```bash
./helper-scripts/create-torrent.sh /path/to/my-media-folder -o my_pack.torrent
```
...
To add your own documents directly:
